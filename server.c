...
void timer(struct timeval, struct timezone);
void error(char *msg)
{
    perror(msg);
    exit(1);
...
{
     int sockfd, portno;
     int timeout=TIMEOUT,timeout_1=TIMEOUT_1;
     int cwnd;
     int sign = 0;
     float pl ;
     float pc ;
     ...
     if (argc < 5) {
         fprintf(stderr,"ERROR, lack of parameter (portno,cwnd,pl,pc)\n");
         exit(1);
     }
             ...
             else
             {
                 ...
                 if(DATA_SIZE<(int)filelen)
                 {
                     if(cwnd<filelen)
                  {
                       ...
                       for(i=0;i<num_of_pkt_in_wnd;i++) //send the initial window
                     {
                       ...
                       timer(tv_1,tz);
                       printf("seq: %d was sent by server\n",firstsend.seq);
                 ...
                 while(1) // receive acks
                 {
                     ...
                     if(newsock)
                     {
                       ...
                       recvpkt = (packet*)rbuffer;
                         if(!cal_loss(pl))//drop pkt upon loss
                         {
                          ...
                          if(!cal_corruption(pc))//drop pkt upon loss
                         {
                             timer(tv_1,tz);
                             printf("ack: %d packet was corrupted\n",recvpkt->ack);
                             continue;
                         }
                         timer(tv_1,tz);
                         printf("ack: %d was received by server\n",recvpkt->ack);
                       ...
                       if(CheckAck(recvpkt,head, cwnd, remainder)>0) // receive a new ack
                       {
                           ...
                           if(head+cwnd>=filelen && sign==0) //last packet
                           {
                               ...
                               int pos_1 = ((int)tail % cwnd)/DATA_SIZE + (remainder>0);
                               //printf("pos1:%d\n",pos_1);
                           ...
                           else if(head+cwnd<filelen)
                            {
                                ...
                                {
                                    int pos = ((int)tail % cwnd)/DATA_SIZE +(remainder>0);
                                    int j=0,flag=0;
                                    for(j=0;j<newpktnum;j++)
                                    {
                     ...
                     else //timeout
                     {
                         ...
                         if((head+cwnd) <= filelen)
                         {
                             ...
                             for(i=0;i<num_of_pkt_in_wnd;i++) //resend the current window
                            {
                             resend.seq = (int)head + i*DATA_SIZE;
                                timer(tv_1,tz);
                                printf("seq: %d was resent by server\n",resend.seq);
                             ...
                             else if(remainder>0)
                             {
                                 ...
                                 int pos_2 = ((int)head % cwnd)/DATA_SIZE +(remainder>0);
                                 int j=0,flag=0;
                         ...
                         {
                             int pos_3 = ((int)head % cwnd)/DATA_SIZE + (remainder>0);
                             //printf("pos_3:%d\n",pos_3);
                     ...
                     tv.tv_sec = timeout;
                     tv.tv_usec = timeout_1;/*end of timeout*/
                 }
                 tv.tv_sec = timeout;
                 tv.tv_usec = timeout_1;/*end of while(1) recv acks*/
             }
         }
     }
...
void timer(struct timeval starttime, struct timezone tz)
{
    struct timeval tv;
    //struct timezone tz;
    //tz = tz_1;
    long cursec,curusec,tsec,tusec;
    gettimeofday (&tv, &tz);
    cursec = tv.tv_sec;
    curusec = tv.tv_usec;
    if(curusec<starttime.tv_usec)
    {
        tusec = curusec + 1000000 - starttime.tv_usec;
        cursec--;
    }
    else
    {
        tusec = curusec - starttime.tv_usec;
    }
    tsec = cursec - starttime.tv_sec;
    printf("time_sec:%3ld time_usec:%6ld ",tsec,tusec);
}
